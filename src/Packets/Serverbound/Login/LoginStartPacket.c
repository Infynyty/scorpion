...
LoginStartPacket *login_start_packet_new() {
	...
	MCVarInt *name_length = writeVarInt(strlen(player));
	NetworkBuffer *player_name = buffer_new();
	buffer_write_little_endian(player_name, player, strlen(player));
...
NetworkBuffer *write_login_packet_to_buffer(LoginStartPacket *packet) {
	...
	buffer_write_little_endian(buffer, packet->player_name->bytes, strlen(player));
